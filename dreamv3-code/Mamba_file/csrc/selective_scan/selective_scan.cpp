...
selective_scan_fwd(const at::Tensor &u, const at::Tensor &delta,   // conv1d的输出(即左侧分支);  SSM要用的Δ矩阵
                  const at::Tensor &A, const at::Tensor &B, const at::Tensor &C,  // SSM的ABC矩阵
                  const c10::optional<at::Tensor> &D_,  // 一个skip值，作为可学习参数，最后会直接加在SSM结果上
                  const c10::optional<at::Tensor> &z_,  // 右侧分支的线性层输出，尚未通过SiLU
                  const c10::optional<at::Tensor> &delta_bias_, // 加在Δ上的偏置向量，不用管
                  bool delta_softplus) {     // 在Δ和A相乘前是否对Δ做softplus处理？默认为True。SoftPlus(x)=log(1+exp(x))
