...
class RMB(nn.Module):
    def __init__(
        ...
        mlp_hidden_dim = int(hidden_dim * mlp_ratio)#来自B站：Ai缝合怪整理的即插即用模块
        ...
        self.scan_len = scan_len#来自B站：Ai缝合怪整理的即插即用模块
    ...
    def _generate_ids(self):
        ...
        self.xs_inverse_ids = xs_inverse_ids#来自B站：Ai缝合怪整理的即插即用模块
        ...
        self.xs_shift_inverse_ids = xs_shift_inverse_ids#来自B站：Ai缝合怪整理的即插即用模块
    ...
    def forward(self, input):
        ...
        self.xs_shift_scan_ids = self.xs_shift_scan_ids.to(input.device)#来自B站：Ai缝合怪整理的即插即用模块
        ...
        x = self.ln_1(input)
        if self.shift_size > 0:#来自B站：Ai缝合怪整理的即插即用模块
...
class VMMAttention(nn.Module):
    def __init__(
        ...
        self.x_proj_weight = nn.Parameter(torch.stack([t.weight for t in self.x_proj], dim=0))  # (K=4, N, inner)
        del self.x_proj#来自B站：Ai缝合怪整理的即插即用模块
        ...
        self.dt_projs_weight = nn.Parameter(torch.stack([t.weight for t in self.dt_projs], dim=0))  # (K=4, inner, rank)
        self.dt_projs_bias = nn.Parameter(torch.stack([t.bias for t in self.dt_projs], dim=0))  # (K=4, inner)
        del self.dt_projs#来自B站：Ai缝合怪整理的即插即用模块
        ...
        self.out_norm = nn.LayerNorm(self.d_inner)#来自B站：Ai缝合怪整理的即插即用模块
        self.out_proj = nn.Linear(self.d_inner, self.d_model, bias=bias, **factory_kwargs)
        self.dropout = nn.Dropout(dropout) if dropout > 0. else None
    ...
    def dt_init(dt_rank, d_inner, dt_scale=1.0, dt_init="random", dt_min=0.001, dt_max=0.1, dt_init_floor=1e-4,
        ...
        if dt_init == "constant":
            nn.init.constant_(dt_proj.weight, dt_init_std)
        elif dt_init == "random":#来自B站：Ai缝合怪整理的即插即用模块
            nn.init.uniform_(dt_proj.weight, -dt_init_std, dt_init_std)
        else:
            raise NotImplementedError

        # Initialize dt bias so that F.softplus(dt_bias) is between dt_min and dt_max
        dt = torch.exp(#来自B站：Ai缝合怪整理的即插即用模块
            torch.rand(d_inner, **factory_kwargs) * (math.log(dt_max) - math.log(dt_min))
            + math.log(dt_min)
        ).clamp(min=dt_init_floor)
        # Inverse of softplus: https://github.com/pytorch/pytorch/issues/72759
        inv_dt = dt + torch.log(-torch.expm1(-dt))
        with torch.no_grad():
            dt_proj.bias.copy_(inv_dt) #来自B站：Ai缝合怪整理的即插即用模块
        # Our initialization would set all Linear.bias to zero, need to mark this one as _no_reinit
        dt_proj.bias._no_reinit = True
    ...
    def D_init(d_inner, copies=1, device=None, merge=True):
        ...
        if copies > 1:
            D = repeat(D, "n1 -> r n1", r=copies)
            if merge:
                D = D.flatten(0, 1)#来自B站：Ai缝合怪整理的即插即用模块
        D = nn.Parameter(D)  # Keep in fp32
        D._no_weight_decay = True
        return D
    ...
    def forward_core(self, x: torch.Tensor,
        ...
        xs = xs.float().view(B, -1, L)#来自B站：Ai缝合怪整理的即插即用模块
        dts = dts.contiguous().float().view(B, -1, L)  # (b, k * d, l)
        Bs = Bs.float().view(B, K, -1, L)  # (b, k, d_state, l)
        Cs = Cs.float().view(B, K, -1, L)  # (b, k, d_state, l)
        out_y = self.selective_scan(
    ...
    def forward(self, x: torch.Tensor,  **kwargs):
        ...
        B, H, W, C = x.shape
        x_dtype = x.dtype #
        xs_scan_ids, xs_inverse_ids = mair_ids_generate(inp_shape=(1, 1, H, W), scan_len=8)  # [B,H,W,C]
        xz = self.in_proj(x)
        x, z = xz.chunk(2, dim=-1)
        ...
        y = self.forward_core(x, (xs_scan_ids.to(x.device), xs_inverse_ids.to(x.device)))
...
# PVMMA CVPR2025 VMMA的二次创新模块
# 输入 B C H W,  输出 B C H W
if __name__ == '__main__':
    input= torch.randn(1, 32, 64, 64).cuda()
    # 创建 RMB 模块
    RMB = RMB(hidden_dim=32,inp_shape=(64,64)).cuda()
    # 将输入图像传入 RMB 模块进行处理
    output = RMB(input)          #需要安装mamba环境才能运行
    # 打印输入和输出的形状
    print('Ai缝合即插即用模块永久更新-RMB_input_size:', input.size())
    print('Ai缝合即插即用模块永久更新-RMB_output_size:', output.size())

    #VMMAttention
    VMMA = VMMAttention(32).cuda()  #需要安装mamba环境才能运行
    # 将输入图像传入 VMMA 模块进行处理
    output = VMMA(input)
    # 打印输入和输出的形状
    print('Ai缝合即插即用模块永久更新-VMMA_input_size:', input.size())
    print('Ai缝合即插即用模块永久更新-VMMA_output_size:', output.size())

    # PVMMA CVPR2025 VMMA的二次创新模块
    # PVMMA = PVMMA(32,64).cuda()
    # # 将输入图像传入 RMB 模块进行处理
    # output = PVMMA(input)
    # 打印输入和输出的形状
    # print('顶会顶刊二次创新模块永久更新在二次创新交流群-PVMMA_input_size:', input.size())
    # print('顶会顶刊二次创新模块永久更新在二次创新交流群-PVMMA_output_size:', output.size())
    #CVPR2025 VMMA模块的二次创新，PVMMA在我的二次创新模块改进交流群，可以直接去发小论文！
    #VMMA二次创新模块只更新二次创新交流，永久更新中
    #二次创新改进商品链接在视频评论区

    print(f'二次创新PVMMA在参数量n_paras和浮点运算FLOPs方面均优于原VMMA模块！')
