...
'''
由于遥感图像的尺度大且物体多样化，
现有基于卷积神经网络（CNN）和变压器（Transformer）
的语义分割方法在捕捉长距离依赖关系方面表现不佳，
或由于复杂的计算复杂性而受到限制。本文提出了CM-UNet，
包含一个基于CNN的编码器，用于提取局部图像特征，以及一个基于Mamba的解码器，
用于聚合和整合全局信息，促进遥感图像的高效语义分割。

具体来说，CSMamba模块被引入作为核心分割解码器，
该模块使用通道和空间注意力作为原始Mamba的门激活条件，以增强特征交互和全局-局部信息融合。
适用于遥感图像
'''
