...
class MSDI_3D(nn.Module):
    def __init__(self, channel):
        super().__init__()

        # 使用 Conv3d 进行3维卷积
        self.convs = nn.ModuleList(
            [nn.Conv3d(c, channel[0], kernel_size=3, stride=1, padding=1) for c in channel]
        )

    def forward(self, x):
        # 输入的参考目标尺寸是第二个张量
        ans = torch.ones_like(x[0])
        target_size = x[0].shape[-3:]  # D, H, W

        for i, x in enumerate(x):
            if x.shape[-3] > target_size[0] or x.shape[-2] > target_size[1] or x.shape[-1] > target_size[2]:
                x = F.adaptive_avg_pool3d(x, target_size)
            elif x.shape[-3] < target_size[0] or x.shape[-2] < target_size[1] or x.shape[-1] < target_size[2]:
...
if __name__ == '__main__':
    # 定义输入3D张量
    # input1 = torch.randn(1, 16, 8, 32, 32)   # B, C, D, H, W
    # input2 = torch.randn(1, 32, 16, 64, 64)
    # input3 = torch.randn(1, 64, 32, 128, 128)
    # input4 = torch.randn(1, 128, 64, 256, 256)

    # 定义输入2D张量
    input1 = torch.randn(1, 16,  32, 32)  # B, C,  H, W
    ...
    # 定义 MSDI_3D 模块，通道数为输入的通道数
    # model = MSDI_3D([16, 32, 64, 128])

    # 定义 MSDI_2D 模块，通道数为输入的通道数
    model = MSDI_2D([16, 32, 64, 128])
