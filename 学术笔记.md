...
### 1. 标准图卷积网络（GCN）层（Kipf & Welling 2017）简化卷积
...
科研创新性排序：新问题新方法（top期刊成果）>老问题新方法（top、主流期刊成果）>新问题老方法（主流期刊成果）>老问题老方法（垃圾期刊成果甚至难以发表）

科研创新点主要有：针对 XX 新问题或特殊工况，提出新的或改进的评价方法；发现新现象，揭示新机制；通过试验现象总结规律，并形成评价方法或公式，可用于解决实际工程问题；开发新算法，可显著提高计算效率；开展新工况下的试验，研究新问题。 

**目前只掌握模块缝合** ；但是模块缝合有简单串并联、内嵌

看文献步骤总结：

解决的问题（痛点）→解决这个问题的方法（这个方法包括建立在谁的基础上，解决了前人问题的什么痛点）→思考创新点（创新点是在这个前人基础的另一个角度的痛点，或者是当前实验的不足的点）
