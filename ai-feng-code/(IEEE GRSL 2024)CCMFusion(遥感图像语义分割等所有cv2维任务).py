...
# https://github.com/sstary/SSRS/tree/main/RS3Mamba
'''
题目：RS3Mamba: Visual State Space Model for Remote Sensing Images Semantic Segmentation  遥感图像语义分割   GRSL 2024

即插即用模块：CCMFusion特征融合模块
我们提出了一种用于特征融合的新型 CCM，它可以桥接来自双分支编码器的特征，使其在遥感图像的表示学习中更加有效。
具体来说，CCM 包括两个并行分支，即全局分支和本地分支。前者用于增强主分支的 Fm，而后者用于处理辅助分支的 Fa。
考虑到 main 分支的特征是通过具有局部属性的卷积运算获得的，我们使用基于窗口的多头自注意力 [15] 来建模长距离依赖。
应该强调的是，此机制还保持线性复杂性。另一方面，考虑到辅助分支的特征是通过具有长距离属性的 VSS 获得的，我们使用卷积来学习局部的细节。
因此，考虑到我们对双分支特性的补全，将这个 fusion 模块命名为协同补全模块。
